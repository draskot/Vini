...
#########################################################################
#
# $Header: /opt/cvs/ADFR/GUI/gridGUI.py,v 1.110.2.11 2017/09/14 22:34:59 annao Exp $
#
# $Id: gridGUI.py,v 1.110.2.11 2017/09/14 22:34:59 annao Exp $
#
    ...
    def run(self):
        while True:
            ...
            if self.process.poll() == None:
                ...
                errors = self.process.stderr.readlines()
                if len(errors):
                    msg = "AutoGrid failed to run in folder %s\nError message:\n%s"%(
                        self.app._gc.folder, ''.join(errors))
...
class ADFRGridMapParametersWidget(QtGui.QWidget):
    ...
    def __init__(self, app, parent=None):
        ...
        self.computeTPointsButton = w = QtGui.QPushButton("compute pockets [AutoSite]")
        layout.addWidget(w)
...
class GridGUI(QtGui.QWidget):
    ...
    def setTtitle(self):
        ...
        self.setWindowTitle("AGFR %s (built: %s) (receptor: %s ligand: %s)"%(
            version.__version__,mglutil.__revision__,rname, lname))
    ...
    def defaultProcessPicking(self, pick):
        ...
        if selections.nbAtoms():
            ...
            self.statusbar.showMessage(msg, timeout=2000)
    ...
    def setCovalentDockingMode(self, val):
        ...
        if  self._covalentBondAtoms[0] is not None and self._covalentBondAtoms[1] is not None:
            ...
            if self._covalentLigAtoms is not None:
                if val: self.showCovLigandAtoms()
    ...
    def getAllADatomTypes(self):
        ...
        AD42atomTypes = {}.fromkeys(['H', 'HD', 'HS', 'C', 'A', 'N', 'NA', 'NS', 'OA', 'OS', 'F',
    ...
    def buildUI(self, pmvViewer=None, parent=None):
        ...
        act = self.showCovBondAtoms = QtGui.QAction(
            ...
            'show atoms forming covalent bond', self)
    ...
    def updateDisplay(self):
        ...
        if self.showReceptorAction.isChecked():
            ...
            dataLabel = 'opacity_lines'
            self.receptor._ag.setData(dataLabel, opac)
            self.pmv.displayLines.refreshDisplay(self.receptor)
        else:
            if self.receptor:
        ...
        if self._MSMSDisplayMode==0 and self.receptor: # hide all residue labels
            ...
            if not self._hasReceptorSurface:
                #from ADFR.utils.fixMaps import getSurface
                #if len(self._clusterItems):
                #    pts = self.getClusterPoints()
                #    tpoints = self.pointsInBox(self.getClusterPoints())[0]
                #    verts, normals, srf, comp = getSurface(self.receptor.select(), tpoints)
                #else:
    ...
    def setFlexRes(self):
        ...
        #print "SetFlexRes: selection string", selStr 
        self.paramsWidget.flexResWidget.setText(selStr)
...
##    @waiting_effects
    ...
    def fillFillsTable(self):
        ...
        for inds, coords, pots, atypes in self._clusters[1:]:
            if len(inds)<self.smallClusterCutOff:
                continue
            ...
            if row < len(self._clusters)-2:
                #data = [row, clProp[row-1][5], len(inds), clProp[row-1][3], clProp[row-1][4]]
    ...
    def computeGrids(self):
        ...
        gc, status = self.agfr.computeGrids(folder, flexResStr,
                                            self._spacing, fixMaps=False,
                                            msms=None, background=True)
        process  = gc.process
        self._gc = gc
    ...
    def mkTrg(self):
        ...
        #self.progress.emit('CREATING ZIP FILE'+self.app.destinationFolder+'.zip') 
        self.paramsWidget.computeGridsButton.setDisabled(False)
        self.agfr.generateTrgFile(self._gc, folder, self._selStr, fixMaps=False)
