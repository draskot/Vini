...
#$Header: /opt/cvs/python/packages/share1.5/mglutil/gui/BasicWidgets/Tk/customizedWidgets.py,v 1.52.10.1 2017/07/26 22:35:41 annao Exp $
#
#$Id: customizedWidgets.py,v 1.52.10.1 2017/07/26 22:35:41 annao Exp $
#
...
class KeySelectable:
    ...
    def key_cb(self, event=None):
        ...
        try:
            l = self.getAllValues()
            i = 0
            _str = self.matchString + event.keysym
            for e in l:
                if str(e)[:self.matchCharIndex]==_str:
                    #print e, i
                    self.clearSelection()
                    self.selectItem(i)
...
class ListChooser(KeySelectableListBox):
    """
    ListChooser presents a list of objects in a scrolled listbox.

    chooserObject <- ListChooser( root, mode='single', title='Choose',
             withComment=0, entries=[], cwcfg=None, command=None,
             commandEvent="<ButtonRelease-1>",
             lbpackcfg={'fill':'both', 'expand':1}, lbwcfg={} )

    - root will be the master in which the widget gets embedded
    - mode can be 'single', 'browse', 'multiple', 'extended'
    - entries can be given as a list of items associated with a description,
    (items,description) or (item,None) if no description is provided.
    double clicking on an entry display the associated comment in a
    textfield if withComment is True.
    - withComment can be True of False
    - command
    - commandEvent
    - cwcfg
    - lbpackcfg
