...
# $Header: /opt/cvs/python/packages/share1.5/mglutil/util/colorUtil.py,v 1.15.18.1 2017/07/26 22:35:43 annao Exp $
#
# $Id: colorUtil.py,v 1.15.18.1 2017/07/26 22:35:43 annao Exp $
#
...
def ToRGB(col, mode="HSV", flag255=0):
    ...
    if mode == 'HEX':
        ...
        if i==0:
            if flag255:
                nCol = numpy.array((v,t,p),'f')*255
                return tuple(nCol)
            return (v,t,p)
        elif i==1:
            if flag255:
                nCol = numpy.array((q,v,p),'f')*255
                return tuple(nCol)
            return (q,v,p)
        elif i==2:
            if flag255:
                nCol = numpy.array((p,v,t),'f')*255
                return tuple(nCol)
            return (p,v,t)
        elif i==3:
            if flag255:
                nCol = numpy.array((p,q,v),'f')*255
                return tuple(nCol)
            return (p,q,v)
        elif i==4:
            if flag255:
                nCol = numpy.array((t,p,v),'f')*255
                return tuple(nCol)
            return (t,p,v)
        elif i==5:
            if flag255:
                nCol = numpy.array((v,p,q),'f')*255
                return tuple(nCol)
            return (v,p,q)
        else:
...
"""
    ...
    if mode == "HEX":
        if col[0] != '#' or len(col)!=7:
            ...
            g = float(eval('0x'+col[3:5]))
            b = float(eval('0x'+col[5:]))
            newCol = numpy.array([r,g,b], 'f')
            assert min(newCol) >=0.0
            assert max(newCol) <=255.0
            col= numpy.array(newCol,'f')/255
    elif mode == "RGB":
        assert len(col)==3
        if flag255: col =  numpy.array(col,'f')/255
    else:
