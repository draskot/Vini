...
#
#########################################################################
#$Header: /opt/cvs/python/packages/share1.5/mglutil/util/packageFilePath.py,v 1.56.8.1 2017/07/26 22:35:43 annao Exp $
#
#$Id: packageFilePath.py,v 1.56.8.1 2017/07/26 22:35:43 annao Exp $

import os, sys, re
import warnings
import user

resourceFolder = user.home + os.sep + ".mgltools2"
...
def getObjectFromFile(absfile, obj, force=False):
...
def getCacheFolder():
    cacheFolder = os.path.join(resourceFolder, 'cache')
...
"""
    ...
    if os.path.isdir(resourceFolder) is False:
        try:
            ...
            resourceFolder = gettempdir() + os.path.sep + ".mgltools2"
    ...
    if resourceFolder:
        try:
            ...
            cacheFolder = resourceFolder
            txt = "Could not create cache folder in resource Folder %s, using resource folder" %(resourceFolder, e)
            warnings.warn(txt)
