...
class RigidfitBodyAligner:
    ...
    def rigidFit(self, mobileCoords):
        ...
        refCoords = numpy.array(refCoords)
        mobileCoords = numpy.array(mobileCoords)

        #
        # 1. Compute centroids:
        refCentroid = numpy.sum(refCoords,0)/ len(refCoords)
        mobileCentroid = numpy.sum(mobileCoords, 0)/ len(mobileCoords)
        ...
        for i in xrange(3):
            for j in xrange(3):
                rot[j][i] = numpy.sum((refCoords[:,i]-refCentroid[i])*
                                      (mobileCoords[:,j]-mobileCentroid[j]),0)

        rotTransposed = numpy.transpose(rot)
        e = numpy.dot(rot, rotTransposed)

        evals, evecs = numpy.linalg.eig(e)
        # MS Feb 2016 evecs are transposed compared to evals, evecs = LinearAlgebra.eigenvectors(e)
        evecs = numpy.transpose(evecs)
        
        ev = numpy.identity(3).astype('d')
    ...
    def transformCoords(self, setCoords):
        ...
        transfoMatrix = numpy.identity(4, 'd')
        ...
        homoCoords = numpy.concatenate((setCoords,
                                          numpy.ones( (len(setCoords), 1), 'd')), 1)
        # 3- apply the transformation matrix to the homogeneous coords.
        transformedCoords = numpy.dot(homoCoords, transfoMatrix)
