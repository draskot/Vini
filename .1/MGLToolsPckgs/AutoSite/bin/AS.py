...
parser.add_argument('-sc', '--cutoffscan', dest='cutoffscan', action='store_true',
                  help="-sc ")
parser.add_argument('-steps', '--steps', dest='steps', default="6",
                  help="-steps STEPS")
...
myprint( 'AutoSite v%d.%d.%d started on %s a %s computer'%(
    AutoSite.__version__, AutoSite.__revision__, AutoSite.__built__, platform.node(), platform.platform()))
...
if args.bestCutoff[0] == "ratio":
    ...
    cmin = args.bestCutoff[1]
    omin = args.bestCutoff[2]
    hmin = args.bestCutoff[3]
else:
...
if args.cutoffscan and args.steps > 1:
    print "Cutoffs scan starting from", cmin,omin,hmin, " to ", cmin/2,omin/2,hmin/2
    steps=int(args.steps)
    tt0=time()
    from AutoSite.compositePoints import CompositePoints
    dcl = gc.bestCutoffClustering(receptor,spacing=spacing,carbon_cutoff=cmin,oxygen_cutoff=omin,hydrogen_cutoff=hmin,nbSteps=steps)
...
clusters, clProp = scoreClusters(receptor, dcl, gc)
if args.cutoffscan and args.steps > 1:
